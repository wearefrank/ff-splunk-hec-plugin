...
@Plugin(name = "FrankSplunkAppender", category = Node.CATEGORY, elementType = Appender.ELEMENT_TYPE, printObject = true)
public class SplunkHecAppender extends AbstractAppender {

	/**
	 * Splunk Http Appender.
	 */
	private HttpEventCollectorSender sender = null;
	private Timer batchEventSenderTimer;
	...
	private SplunkHecAppender(final String name, final String url, final String token, final String channel, final String type, final String env, final String source,
			final String sourcetype, final String messageFormat, final String host, final String index, final Filter filter,
			final Layout<? extends Serializable> layout, final boolean includeLoggerName, final boolean includeThreadName, final boolean includeMDC,
			final boolean includeException, final boolean includeMarker, final boolean ignoreExceptions, long batchInterval, long batchCount, long batchSize,
			long retriesOnError, String sendMode, String middleware, final String disableCertificateValidation, final String eventBodySerializer,
			final String eventHeaderSerializer, HttpEventCollectorSender.TimeoutSettings timeoutSettings) {
		...
		metadata.put("env", env != null ? env : "");

		// batchInterval == DELAY; Always pass 0 to avoid leaks upon close, manage the timer in here instead.
		this.sender = new HttpEventCollectorSender(url, token, channel, type, 0, batchCount, batchSize, sendMode, metadata, timeoutSettings);
		...
		if (batchInterval > 0) {
			...
			batchEventSenderTimer = new Timer(true);
			batchEventSenderTimer.scheduleAtFixedRate(this.sender, batchInterval, batchInterval);
		}
	}
	...
	// @formatter:off
		...
		return new SplunkHecAppender(
				name, url, token,  channel, type, env,
	...
	public boolean stop(long timeout, TimeUnit timeUnit) {
		if (batchEventSenderTimer != null) {
			this.batchEventSenderTimer.cancel();
		}
